...
public interface IRelationManager <C extends IAppClient,V>{
	 ...
	 * @return
	 */
	ICache<AccountZoo> getUserToAccountZooCache();
	...
	/**
	 * 接口设计的不好，session参数不通用
	 * 返回当前用户的间接好友，就是朋友的朋友(支持到两级)
	 * @param 用户id
	 * @param 查询分页用的上下文，每次查询会修改这个对象作为下次查询的输入
	 * @return
	 */
	List<User> getIndirectFriendsByUser(String uid,FriendSecondhandQuerySession session) throws ServiceException;
